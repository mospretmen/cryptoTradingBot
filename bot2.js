...
setInterval(function(){
  ...
  binance.candlesticks("XRPETH", "15m", (error, ticks, symbol) => {
    ...
    let [time, open, high, low, close, volume, closeTime, assetVolume, trades, buyBaseVolume, buyAssetVolume, ignored] = last_tick;
      ...
      var simpleMovingAverage100 = SMA.calculate({period : 100, values : array100Period});
      ...
      (async function data() {
        ...
        let bidAsk = await asyncData.getBidAsk.XRP();
        let candlesticks = await asyncData.getCandlesticks.XRP();
        let depth = await asyncData.getMarketDepth.XRP();
        ...
        if(+ticks[99][4] < lower && +rsi < 38 && bollingerSpread < bolSpreadParameter) {
          ...
          setTimeout(function(){  
            ...
            binance.buy("XRPETH", qtyTrade1, Number(result.bidAsk.bidPrice) + +0.00000001);
        },500);
          ...
          setTimeout(function(){  
            ...
            binance.buy("XRPETH", qtyTrade1 * 3, Number(result.bidAsk.bidPrice) + +0.0000001);
        },500);
        ...
        } else if(+ticks[99][4] >= upper && +rsi > 61) {
          setTimeout(function(){    
            binance.cancelOrders("XRPETH", (error, response, symbol) => {
              console.log(symbol+" cancel response:", response);
            });
        ...
        // ADD more BNB for transaction fees.   
          ...
          console.log('============================================================');      
          console.log(new Date().toLocaleString()); console.log(colors.cyan('Waiting for trade... => ' + symbol).bold)}
          console.log('------------------------------------------------------------');
            if( +close > +simpleMovingAverage100[simpleMovingAverage100.length -1]) {console.log('^^^^^^^^^^^^^^^^^^^^^^^^ Trend is UP ^^^^^^^^^^^^^^^^^^^^^^^^'.bgGreen + "\n")} 
          ...
          if(Number(close) > bollingerBands[bollingerBands.length -1].upper) {console.log("Last Close: " + colors.green(close));
...
setInterval(function(){
  //GET Chart Data
  binance.candlesticks("EOSETH", "5m", (error, ticks, symbol) => {
    // console.log("candlesticks()", ticks, ticks[19][4]);
    let last_tick = ticks[ticks.length - 1];
    let [time, open, high, low, close, volume, closeTime, assetVolume, trades, buyBaseVolume, buyAssetVolume, ignored] = last_tick;
      ...
      (async function data() {
        ...
        let balances = await asyncData.getBalances();
        let prices = await asyncData.getPriceData();
        let bidAsk = await asyncData.getBidAsk.EOS();
        // let candlesticks = await asyncData.getCandlesticks.EOS();
        // let depth = await asyncData.getMarketDepth.EOS();
        ...
        if(+ticks[99][4] < lower && +rsi < 38 && bollingerSpread < 1.04) {
          ...
          setTimeout(function(){    
            ...
            binance.sell("EOSETH", qtyTrade1, +(Number(result.bidAsk.askPrice) * 1.002).toFixed(6));
        },5000);
    ...
    if(Number(close) > bollingerBands[bollingerBands.length -1].upper) {console.log("Last Close: " + colors.green(close));
      ...
      console.log("Last Close: " + Number(ticks[99][4]).toFixed(6));
    }
    if (bullish(fivePeriodCandlestickInput) === true) {
      console.log(`Candlestick Pattern Bullish?: ${colors.green(bullish(fivePeriodCandlestickInput))}`);
    } else {
    ...
    console.log("SMA 100 Period: " + (simpleMovingAverage100[simpleMovingAverage100.length -1]).toFixed(6));
    console.log("Upper Limit @Sigma Lvl. " + inputBB.stdDev + " = " + upper.toFixed(6));
    console.log("Lower Limit @Sigma Lvl. " + inputBB.stdDev + " = " + lower.toFixed(6));
    console.log("Bollinger Bands Spread: " + colors.yellow((((bollingerSpread)-1)*100).toFixed(2) + " %"));
    console.log('RSI: ' + colors.yellow(rsi));
    console.log("ROC 5,10,20,40,99 period: " + colors.yellow(`${roc5.toFixed(2)}, ${roc10.toFixed(2)}, ${roc20.toFixed(2)}, ${roc40.toFixed(2)}, ${roc99.toFixed(2)} %`));
    console.log(`Last Volume: ${colors.yellow(lastVolume)}, Average Volume: ${colors.yellow(averageVolume.toFixed(0))}`);
    ...
    + `Count Buy Trades: ${result.tradeHistoryData[2]} \n`
    + `Count Sell Trades: ${result.tradeHistoryData[5]} \n`
    + `Total Qty Bought: ${result.tradeHistoryData[1].toFixed(0)} \n`
    + `Total Qty Sold: ${result.tradeHistoryData[4].toFixed(0)} \n`
    + `Weighted Buy Price: ${result.tradeHistoryData[0]} \n`
    + `Weighted Sell Price: ${result.tradeHistoryData[3]} \n`
    + 'Total Profit/Loss: ' + ((+result.tradeHistoryData[3] - +result.tradeHistoryData[0]) * Math.min(Number(math.sum(+result.tradeHistoryData[4])), Number(math.sum(+result.tradeHistoryData[1]))) ).toFixed(4) + ' ETH');
...
app.get('/', (req, res) => {
    ...
    res.render('index.ejs', {test: mongoose.tradingbot.tradehistory});
});
